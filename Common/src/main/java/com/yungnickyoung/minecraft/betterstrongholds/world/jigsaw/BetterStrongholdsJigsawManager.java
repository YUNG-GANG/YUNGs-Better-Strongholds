...
public class BetterStrongholdsJigsawManager {
    public static Optional<PieceGenerator<BetterStrongholdFeatureConfiguration>> assembleJigsawStructure(
            PieceGeneratorSupplier.Context<BetterStrongholdFeatureConfiguration> jigsawContext,
        ...
        BetterStrongholdFeatureConfiguration config = jigsawContext.config();
        ...
        // Get starting pool
        StructureTemplatePool structureTemplatePool = registry.get(config.startPool);
        ...
        return Optional.of((structurePiecesBuilder, context) -> {
            ...
            if (config.maxDepth <= 0) { // Realistically this should never be true. Why make a jigsaw config with a non-positive size?
                return;
            }
            ...
            Placer placer = new Placer(registry, config.maxDepth, pieceFactory, chunkGenerator, structureManager, pieces, worldgenRandom);
    ...
    public static final class Placer {
        ...
        public Placer(
            ...
            this.pieceCounts.put(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "rooms/grand_library"), BetterStrongholdsCommon.CONFIG.pieceSettings.grandLibraryMaxCount);
            this.pieceCounts.put(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "rooms/library_md"), BetterStrongholdsCommon.CONFIG.pieceSettings.smallLibraryMaxCount);
            this.pieceCounts.put(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "rooms/prison_lg"), BetterStrongholdsCommon.CONFIG.pieceSettings.prisonMaxCount);
            this.pieceCounts.put(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "rooms/cmd_acarii"), BetterStrongholdsCommon.CONFIG.pieceSettings.cmdAcariiMaxCount);
            this.pieceCounts.put(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "rooms/cmd_yung"), BetterStrongholdsCommon.CONFIG.pieceSettings.cmdYungMaxCount);
            this.pieceCounts.put(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "rooms/treasure_room_lg"), BetterStrongholdsCommon.CONFIG.pieceSettings.treasureRoomMaxCount);
            this.pieceCounts.put(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "portal_rooms/portal_room"), BetterStrongholdsCommon.CONFIG.pieceSettings.portalRoomMaxCount);
            this.pieceCounts.put(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "rooms/armoury_md"), BetterStrongholdsCommon.CONFIG.pieceSettings.armouryLargeRoomMaxCount);
            this.pieceCounts.put(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "rooms/armoury_sm"), BetterStrongholdsCommon.CONFIG.pieceSettings.armourySmallRoomMaxCount);
        ...
        public void processPiece(
            ...
            for (StructureTemplate.StructureBlockInfo jigsawBlock : pieceJigsawBlocks) {
                ...
                if (!(poolOptional.isPresent() && (poolOptional.get().size() != 0 || Objects.equals(jigsawBlockPoolId, Pools.EMPTY.location())))) {
                    BetterStrongholdsCommon.LOGGER.warn("Empty or nonexistent pool: {}", jigsawBlockPoolId);
                    continue;
                }
                ...
                if (!(fallbackOptional.isPresent() && (fallbackOptional.get().size() != 0 || Objects.equals(jigsawBlockFallback, Pools.EMPTY.location())))) {
                    BetterStrongholdsCommon.LOGGER.warn("Empty or nonexistent fallback pool: {}", jigsawBlockFallback);
                    continue;
                }
        ...
        private StructurePoolElement processList(
            ...
            while (candidatePieces.size() > 0 && totalWeightSum > 0) {
                ...
                if (this.pieceCounts.get(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "portal_rooms/portal_room")) > 0) { // Condition 2
                    for (int i = 0; i < candidatePieces.size(); i++) {
                        ...
                        if (((SinglePoolElementAccessor) candidatePiece).getTemplate().left().get().equals(new ResourceLocation(BetterStrongholdsCommon.MOD_ID, "portal_rooms/portal_room"))) { // Condition 1
